...
배포 후 실제 URL을 아래 환경에서 열어 확인합니다. 로컬 빌드와 브라우저 폭 점검만으로는 카카오톡/모바일 OS 동작을 완전히 대체할 수 없습니다.

1. iPhone Safari에서 링크를 열고 360px~430px 기준 레이아웃, 스크롤, 인트로 전환을 확인합니다.
2. Android Chrome에서 갤러리 모달, 계좌 복사, 링크 복사, 캘린더 저장을 확인합니다.
3. 카카오톡 인앱 브라우저에서 링크 열림, 카카오 공유 fallback, OG 제목/설명/이미지를 확인합니다.
4. 네이버앱 또는 삼성인터넷이 가능하면 지도 버튼과 공유 버튼을 추가로 확인합니다.
5. 지도 버튼은 카카오맵, 네이버지도, 티맵, 구글맵 순서로 실제 앱/웹 이동 여부를 확인합니다.
6. 확인 결과는 `docs/QA_CHECKLIST.md`의 MW-15 섹션에 기록합니다.
