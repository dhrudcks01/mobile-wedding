...
### 목표
비어있는 wedding.ts 데이터를 실제 청첩장 정보로 쉽게 바꿀 수 있게 구조와 주석을 정리한다.

### 범위
- src/data/wedding.ts 실제 입력 예시 보강
- 빈 문자열 fallback 정리
- README에 실제 정보 변경 방법 추가
- 전화번호/계좌번호 공개 주의 문구 추가

### 완료 기준
- [x] 신랑/신부/날짜/장소/계좌/지도/문구를 어디서 바꾸는지 명확함
- [x] 빈 값이면 해당 UI가 자연스럽게 숨겨짐
- [x] pnpm lint 통과
- [x] pnpm build 통과
...
### 범위
- public/images 예시 구조 정리
- hero, og-image, gallery 이미지 경로 정리
- 이미지 없을 때 fallback UI 개선
- README에 사진 넣는 방법 추가
...
### 목표
모바일 접속 시 2~3초간 이름/문구가 보이는 인트로 화면을 보여준 뒤 메인 청첩장으로 전환한다.
...
- 신랑/신부 이름, 짧은 문구 표시
- fade-out 후 메인 화면 표시
- prefers-reduced-motion 사용자는 애니메이션 최소화

### 완료 기준
- [x] 첫 접속 시 인트로가 보임
- [x] 약 2.5초 후 메인 청첩장 표시
- [x] 새로고침 시 정상 동작
- [x] 모바일에서 화면 깜빡임 심하지 않음
- [x] pnpm lint/build 통과
...
- 카카오톡 인앱 브라우저
- 네이버앱/삼성인터넷 가능하면 확인
- 링크 복사, 계좌 복사, 지도 이동, 캘린더 저장, 공유 확인

### 완료 기준
- [x] 360px, 390px, 430px 레이아웃 정상
- [x] 카카오톡에서 링크 열림
- [x] 지도 버튼 정상
- [x] 계좌 복사 정상
- [x] 캘린더 저장 정상
...
### 참고
- 로컬 환경에서는 실제 iPhone/Android/카카오톡 인앱 브라우저를 완전히 대체할 수 없으므로 `docs/MW15_MOBILE_QA.md` 기준으로 배포 URL에서 최종 실기기 확인을 진행한다.
